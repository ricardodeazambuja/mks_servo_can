...
def get_version_from_init():
    ...
    init_py_path = os.path.join(os.path.dirname(__file__), 'mks_servo_can', '__init__.py')
    try:
        ...
        raise RuntimeError(f"{init_py_path} not found. Ensure you are in the correct directory.")
...
try:
    with open('../README.md', 'r', encoding='utf-8') as f: # Path relative to this setup.py
        long_description = f.read()
except FileNotFoundError:
    # Fallback if README.md is not found at the expected relative path
    long_description = "Python library to control MKS SERVO42D/57D motors via CAN bus."

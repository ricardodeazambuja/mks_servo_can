...
from .kinematics import Kinematics  # Default kinematics
...
class Axis:
    ...
    def __init__(
        ...
        self.default_speed_user = (
            0  # User units, will be set by set_default_speed
        )
        self.default_accel_user = (
            0  # User units/s^2 placeholder, accel is more complex
        )
        ...
        # State variables
        self._current_position_steps: Optional[int] = (
            None  # In motor steps/encoder counts
        )
    ...
    async def initialize(
        ...
        """
        logger.info(
            f"Initializing axis '{self.name}' (CAN ID: {self.can_id:03X})..."
        )
        try:
            ...
            self._is_enabled = await self._low_level_api.read_en_pin_status(
                self.can_id
            )
            logger.info(
            ...
            if home:
                if (
                    not self._is_calibrated and calibrate
                ):  # Only home if calibration was successful
                    await self.home_axis()
                elif (
                    self._is_calibrated
                ):  # If already considered calibrated (e.g. from previous session)
                    await self.home_axis()
                else:
                    logger.warning(
                        f"Axis '{self.name}': Skipping homing because calibration is required but not performed/successful."
                    )
    ...
    async def set_currents(
        ...
        holding_current_percentage_code: Optional[int] = None,
    ) -> None:
        """Sets the working current (mA) and optionally the holding current percentage code."""
        # Add motor_type specific max current checks
        max_curr = (
            const.MAX_CURRENT_SERVO57D
            if self.motor_type == const.MOTOR_TYPE_SERVO57D
            else const.MAX_CURRENT_SERVO42D
        )
        if not (0 <= working_current_ma <= max_curr):
            raise ParameterError(
                f"Working current {working_current_ma}mA out of range for {self.motor_type} (0-{max_curr}mA)."
            )

        logger.info(
            f"Axis '{self.name}': Setting working current to {working_current_ma}mA."
        )
        await self._low_level_api.set_working_current(
            self.can_id, working_current_ma
        )

        if holding_current_percentage_code is not None:
            if not (0x00 <= holding_current_percentage_code <= 0x08):
                raise ParameterError(
                    "Holding current percentage code out of range (0-8)."
                )
            logger.info(
                f"Axis '{self.name}': Setting holding current percentage code to {holding_current_percentage_code}."
            )
            await self._low_level_api.set_holding_current_percentage(
                self.can_id, holding_current_percentage_code
            )

    async def set_microstepping(
        self, microsteps: int, interpolation: Optional[bool] = None
    ) -> None:
        ...
        logger.info(
            f"Axis '{self.name}': Setting microstepping to {microsteps}."
        )
        await self._low_level_api.set_subdivision(self.can_id, microsteps)
        if interpolation is not None:
            logger.info(
                f"Axis '{self.name}': Setting subdivision interpolation to {interpolation}."
            )
            await self._low_level_api.set_subdivision_interpolation(
                self.can_id, interpolation
            )

    async def configure_homing_parameters(
        self,
        home_trig_low: bool,
        home_dir_ccw: bool,
        home_speed_rpm: int,
        ...
        nolimit_homing_current_ma: Optional[int] = None,
    ) -> None:
        ...
        if no_limit_home_mode:
            if (
                nolimit_reverse_angle_pulses is None
                or nolimit_homing_current_ma is None
            ):
                raise ParameterError(
                    "For no-limit homing, reverse angle pulses and homing current must be provided."
                )
            await self._low_level_api.set_nolimit_home_parameters(
                self.can_id,
                nolimit_reverse_angle_pulses,
                nolimit_homing_current_ma,
            )
    ...
    async def calibrate_encoder(self) -> None:
        ...
        if self._is_enabled:
            logger.warning(
                f"Axis '{self.name}': Motor is enabled. Disabling temporarily for calibration is often recommended but not enforced by this library."
            )
    ...
    async def home_axis(
        ...
        if not self._is_calibrated:
            logger.warning(
                f"Axis '{self.name}': Attempting to home non-calibrated axis. Calibration is usually required first."
            )
                ...
                if wait_for_completion:
                    ...
                    while time.monotonic() - start_time < timeout:
                        ...
                        current_motor_status = (
                            await self._low_level_api.query_motor_status(
                                self.can_id
                            )
                        )
                        ...
                        if current_motor_status != const.MOTOR_STATUS_HOMING:
                            ...
                            # This is not robust.
                            logger.info(
                                f"Axis '{self.name}': Homing process appears complete (no longer in homing state)."
                            )
                            ...
                            return
                    raise HomingError(
                        f"Axis '{self.name}': Timeout waiting for homing completion.",
                        can_id=self.can_id,
                    )
                else:  # Not waiting for completion
                    self._is_homed = False  # Status is uncertain
                    logger.info(
                        f"Axis '{self.name}': Homing initiated, not waiting for completion."
                    )
                    return
            elif (
                initial_status == const.HOME_SUCCESS
            ):  # 0x02 (if it completes immediately)
                self._is_homed = True
                self._error_state = None
                self._current_position_steps = 0
                logger.info(
                    f"Axis '{self.name}': Homing successful (immediate)."
                )
                return
            else:  # Includes HOME_FAIL (0x00)
                raise HomingError(
                    f"Axis '{self.name}': Homing command failed to start or reported failure immediately. Status: {initial_status}",
                    error_code=initial_status,
                    can_id=self.can_id,
                )
    ...
    async def set_current_position_as_zero(self) -> None:
        ...
        self._current_position_steps = 0
        self._is_homed = (
            True  # Usually, setting zero implies a known (homed) state.
        )
    ...
    async def _execute_move(
        ...
        move_timeout = (
            const.CAN_TIMEOUT_SECONDS * 5
        )  # Example: 5s for a move, adjust as needed
        ...
        try:
            ...
            if initial_status == const.POS_RUN_STARTING:
                logger.debug(
                    f"Axis '{self.name}': Move ({command_const:02X}) started. Waiting for completion..."
                )
                ...
                if final_status == success_status:  # e.g., POS_RUN_COMPLETE
                    logger.info(
                        f"Axis '{self.name}': Move ({command_const:02X}) completed successfully."
                    )
                    ...
                    self._active_move_future.set_exception(
                        LimitError(
                            msg, error_code=final_status, can_id=self.can_id
                        )
                    )
                ...
                initial_status == success_status
            ):  # Completed in the first response
                logger.info(
                    f"Axis '{self.name}': Move ({command_const:02X}) completed successfully (immediate)."
                )
                self._active_move_future.set_result(True)
            ...
            logger.error(
                f"Axis '{self.name}': Error during move ({command_const:02X}): {e}"
            )
            if not self._active_move_future.done():
                self._active_move_future.set_exception(e)
        finally:
            if (
                self._active_move_future.done()
            ):  # Check if it was set by success/specific error
                pass  # Don't overwrite
            else:  # Should not happen if logic above is correct
                self._active_move_future.set_exception(
                    MKSServoError(
                        f"Move future for {command_const:02X} ended without explicit result/error."
                    )
                )
    ...
    async def move_to_position_abs_pulses(
        ...
        sp = (
            speed_param if speed_param is not None else self.default_speed_param
        )
        ac = (
            accel_param if accel_param is not None else self.default_accel_param
        )
        logger.info(
        ...
        cmd_func = (
            lambda: self._low_level_api.run_position_mode_absolute_pulses(
                self.can_id, sp, ac, target_pulses
            )
        )
        await self._execute_move(
            cmd_func, const.CMD_RUN_POSITION_MODE_ABSOLUTE_PULSES
        )
    ...
    async def move_to_position_abs_user(
        ...
        if speed_user is not None:
            mks_speed_param = self.kinematics.user_speed_to_motor_speed(
                speed_user
            )
        else:
        ...
        logger.info(
            f"Axis '{self.name}': Moving to user position {target_pos_user} ({self.kinematics.units}) "
    ...
    async def move_relative_pulses(
        ...
        cmd_func = (
            lambda: self._low_level_api.run_position_mode_relative_pulses(
                self.can_id, direction_ccw, sp, ac, num_pulses
            )
        )
        await self._execute_move(
            cmd_func, const.CMD_RUN_POSITION_MODE_RELATIVE_PULSES
        )
        if wait and self._active_move_future:
            await self._active_move_future
    ...
    async def move_relative_user(
        ...
        )  # Use default MKS accel param

        direction_ccw = (
            relative_steps > 0
        )  # Positive steps for CCW if kinematics define it so.
        ...
        logger.info(
            f"Axis '{self.name}': Moving relative by {relative_dist_user} {self.kinematics.units} "
    ...
    async def set_speed_user(
        ...
        direction_ccw = (
            speed_user > 0
        )  # Assuming positive user speed means CCW for 0xF6.

        if speed_user == 0:
            logger.info(
                f"Axis '{self.name}': Stopping continuous speed mode (AccelP: {mks_accel_param})."
            )
            await self._low_level_api.stop_speed_mode(
                self.can_id, mks_accel_param
            )
        else:
            logger.info(
                f"Axis '{self.name}': Setting continuous speed to {speed_user} {self.kinematics.units}/s "
    ...
    async def stop_motor(
        self, deceleration_param: Optional[int] = None
    ) -> None:
        ...
        decel = (
            deceleration_param
            if deceleration_param is not None
            else self.default_accel_param
        )
        logger.info(
            f"Axis '{self.name}': Commanding stop (decel_param: {decel})."
        )
        try:
            ...
            if self._active_move_future and not self._active_move_future.done():
                self._active_move_future.cancel(
                    "Motor stopped by explicit command"
                )
        except MKSServoError as e:
            logger.warning(
    ...
    def is_enabled(self) -> bool:
        ...
        return (
            self._is_enabled
        )  # For quick check, or query if needed: await self.read_en_status()
    ...
    async def get_current_position_steps(self) -> int:
        ...
        pos_steps = await self._low_level_api.read_encoder_value_addition(
            self.can_id
        )
    ...
    async def get_current_speed_user(self) -> float:
        ...
        if self.kinematics.__class__.__name__ == "LinearKinematics":
            ...
            user_speed = (
                output_revs_per_sec
                * self.kinematics.degrees_per_output_revolution
            )
            ...
            theta_out_deg = current_steps / (
                self.kinematics.effective_steps_per_output_revolution / 360.0
            )
            theta_out_rad = math.radians(theta_out_deg)

            output_angular_speed_rad_per_sec = math.radians(
                output_revs_per_sec * 360.0
            )

            user_speed = (
                self.kinematics.arm_length
                * math.cos(theta_out_rad)
                * output_angular_speed_rad_per_sec
            )
        else:
            logger.warning(
                f"User speed calculation not fully implemented for kinematics type {self.kinematics.__class__.__name__}"
            )
    ...
    async def get_status_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "position_units": (
                self.kinematics.units
                if hasattr(self.kinematics, "units")
                else "degrees"
            ),
            # "speed_rpm": speed_rpm,
            ...
            "motor_status_str": (
                const.MOTOR_STATUS_MAP.get(raw_motor_status, "Unknown")
                if hasattr(const, "MOTOR_STATUS_MAP")
                else "N/A"
            ),  # Add MOTOR_STATUS_MAP to consts
            "error_state": (
                str(self._error_state) if self._error_state else None
            ),
            "active_move_in_progress": self._active_move_future is not None
            and not self._active_move_future.done(),
        }
    ...
    async def set_io_output(
        self, out1_value: Optional[int] = None, out2_value: Optional[int] = None
    ) -> None:
        ...
        if self.motor_type not in [
            const.MOTOR_TYPE_SERVO57D
        ]:  # Add other types if they get outputs
            raise ConfigurationError(
                f"Motor type {self.motor_type} may not support direct IO output control via CMD 0x36."
            )
        await self._low_level_api.write_io_port(
            self.can_id, out1_value=out1_value, out2_value=out2_value
        )
    ...
    async def read_parameter(self, parameter_command_code: int) -> bytes:
        ...
        _, param_data = await self._low_level_api.read_system_parameter(
            self.can_id, parameter_command_code
        )
        return param_data

    async def write_parameter_raw(
        self, command_code: int, data_bytes: list[int]
    ):
        """
        ...
        logger.warning(
            f"Axis '{self.name}': Writing raw parameter CMD {command_code:02X} with data {data_bytes}. Use with caution."
        )
        response = await self._low_level_api._send_command_and_get_response(
            self.can_id,
            command_code,
            data=data_bytes,
            expected_dlc=3,  # Assuming simple status response
        )
        if response.data[1] != const.STATUS_SUCCESS:
            raise MotorError(
                f"Raw write parameter CMD {command_code:02X} failed.",
                error_code=response.data[1],
                can_id=self.can_id,
            )
        logger.info(
            f"Axis '{self.name}': Raw parameter CMD {command_code:02X} written successfully."
        )
        ...
        if self._active_move_future and not self._active_move_future.done():
            try:
                await asyncio.wait_for(
                    self._active_move_future, timeout=timeout
                )
            except asyncio.TimeoutError:
                ...
                MKSServoError
            ):  # Catch errors set on the future by _execute_move

...
class EccentricKinematics(Kinematics):
    ...
    """
    def __init__(self, steps_per_revolution: int, arm_length: float,
                 gear_ratio: float = 1.0, max_displacement: float = 0.0,
                 units: str = "mm"):
        """
        ...
        self.units = units
        self.max_displacement = max_displacement if max_displacement > 0 else arm_length

        # Internal: steps per degree of the motor shaft itself (before output gearing)
        self._motor_steps_per_degree_motor_shaft = self.steps_per_revolution / 360.0
    ...
    def _user_to_motor_angle_deg(self, user_value: float) -> float:
        ...
        if self.arm_length == 0: return 0.0 # Avoid division by zero
    ...
    def user_to_steps(self, user_value: float) -> int:
        ...
        steps = round(motor_output_angle_deg * (self.effective_steps_per_output_revolution / 360.0))
        return steps
    ...
    def steps_to_user(self, steps_value: int) -> float:
        ...
        """
        motor_output_angle_deg = steps_value / (self.effective_steps_per_output_revolution / 360.0)
    ...
    def user_speed_to_motor_speed(self, user_speed: float) -> int:
        ...
        output_angular_speed_deg_per_sec = math.degrees(output_angular_speed_rad_per_sec)
    ...
    def motor_speed_to_user_speed(self, motor_speed_param: int) -> float:
        ...
        output_angular_speed_rad_per_sec = math.radians(output_angular_speed_deg_per_sec)

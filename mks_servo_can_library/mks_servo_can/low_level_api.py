...
try:
    ...
    class CanMessage: # type: ignore
        def __init__(self, arbitration_id=0, data=None, is_extended_id=False, dlc=0): # Added dlc
            self.arbitration_id = arbitration_id
            self.data = data if data is not None else b'' # Ensure bytes
            self.is_extended_id = is_extended_id
            self.dlc = dlc if data is None else len(data) # Correct DLC calculation
            self.timestamp = 0.0
            self.channel = None
            self.is_rx = True
...
class LowLevelAPI:
    ...
    async def _send_command_no_response(
        ...
        if not (const.BROADCAST_ADDRESS <= can_id <= 0x7FF):
            raise ParameterError(f"Invalid CAN ID: {can_id}. Must be 0-2047 (0x7FF).")
        payload_data = [command_code]
        if data: payload_data.extend(data)
    ...
    async def read_encoder_value_addition(self, can_id: int) -> int:
        ...
        if val_bytes[5] & 0x80: val_bytes += b'\xff\xff' # Sign extend for negative
        else: val_bytes += b'\x00\x00' # Pad for positive
    ...
    async def read_io_status(self, can_id: int) -> Dict[str, int]:
        ...
        return {
            "IN_1": (status_byte >> 0) & 0x01, "IN_2": (status_byte >> 1) & 0x01,
            "OUT_1": (status_byte >> 2) & 0x01, "OUT_2": (status_byte >> 3) & 0x01,
    ...
    async def read_shaft_angle_error(self, can_id: int) -> int:
        response = await self._send_command_and_get_response(can_id, const.CMD_READ_SHAFT_ANGLE_ERROR, expected_dlc=6)
    ...
    async def read_power_on_zero_status(self, can_id: int) -> int:
        ...
        if status == 2: # Explicit failure code for this command
            logger.warning(f"Power on zero status for CAN ID {can_id} reported failure (status 2).")
        return status
    ...
    async def release_stall_protection(self, can_id: int) -> bool:
        ...
        if response.data[1] == 0x01: return True
        elif response.data[1] == 0x00:
            logger.warning(f"Release stall protection failed for CAN ID {can_id} (status 0).")
            return False # This is a failure specific to this command's meaning
        else: raise CommandError(f"Unexpected status from release_stall_protection: {response.data[1]}")
    ...
    async def calibrate_encoder(self, can_id: int) -> None:
        ...
        if status == const.STATUS_CALIBRATING_FAIL: # 2
            ...
            raise CalibrationError("Encoder calibration started but did not confirm success in single response (status 'calibrating').", error_code=status, can_id=can_id)
        else:
    ...
    async def set_work_mode(self, can_id: int, mode: int) -> None:
        if not (0 <= mode <= 5): raise ParameterError(f"Invalid work mode: {mode}.")
        response = await self._send_command_and_get_response(can_id, const.CMD_SET_WORK_MODE, data=[mode], expected_dlc=3)
        if response.data[1] != const.STATUS_SUCCESS: # Check specific success status for this command
            raise MotorError(f"Set work mode to {mode} failed for CAN ID {can_id}. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)

    async def set_working_current(self, can_id: int, current_ma: int) -> None:
        if not (0 <= current_ma <= 5200): raise ParameterError(f"Invalid working current: {current_ma}mA.")
        ...
        if response.data[1] != const.STATUS_SUCCESS:
            raise MotorError(f"Set working current to {current_ma}mA failed. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)

    async def set_holding_current_percentage(self, can_id: int, percentage_code: int) -> None:
        if not (0x00 <= percentage_code <= 0x08): raise ParameterError(f"Invalid holding current code: {percentage_code}.")
        ...
        if response.data[1] != const.STATUS_SUCCESS:
            raise MotorError(f"Set holding current percentage failed. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)

    async def set_subdivision(self, can_id: int, microsteps: int) -> None:
        if not (1 <= microsteps <= 255): raise ParameterError(f"Invalid microsteps: {microsteps}.")
        response = await self._send_command_and_get_response(can_id, const.CMD_SET_SUBDIVISION, data=[microsteps], expected_dlc=3)
        if response.data[1] != const.STATUS_SUCCESS:
            raise MotorError(f"Set subdivision failed. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)
    ...
    async def enable_motor(self, can_id: int, enable: bool) -> None:
        ...
        if response.data[1] != const.STATUS_SUCCESS: # 0x01 for success
            raise MotorError(f"Enable motor command (enable={enable}) failed for CAN ID {can_id}. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)
    ...
    async def _run_motor_command(self, can_id: int, command_code: int, data: List[int]) -> int:
        ...
        if status == const.POS_RUN_FAIL: # 0x00
            raise MotorError(f"Motor run command {command_code:02X} failed to start (status 0).", error_code=status, can_id=can_id)
    ...
    async def set_en_pin_active_level(self, can_id: int, level_code: int) -> None:
        ...
        if response.data[1] != const.STATUS_SUCCESS:
            raise MotorError(f"Set En pin active level to {level_code} failed. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)
    ...
    async def set_current_axis_to_zero(self, can_id: int) -> None:
        ...
        if response.data[1] != const.STATUS_SUCCESS:
            raise MotorError(f"Set current axis to zero failed. Status: {response.data[1]}", error_code=response.data[1], can_id=can_id)
    ...
    async def run_position_mode_relative_pulses(self, can_id: int, ccw_direction: bool, speed: int,
        ...
        byte2 = 0x00
        if not ccw_direction: byte2 |= 0x80 # Manual: b7=dir (0=CCW, 1=CW). So if NOT ccw (i.e. CW), set bit.
        byte2 |= ((speed >> 8) & 0x0F)
        byte3 = speed & 0xFF
        byte4 = acceleration
    ...
    async def run_position_mode_absolute_pulses(self, can_id: int, speed: int,
        ...
        if not (0 <= speed <= 3000): raise ParameterError("Speed out of range (0-3000).")
        if not (0 <= acceleration <= 255): raise ParameterError("Acceleration out of range.")
        if not (-0x800000 <= absolute_pulses <= 0x7FFFFF): raise ParameterError("Absolute pulses out of range for int24_t.")
        speed_bytes = list(struct.pack('<H', speed))
        acc_byte = acceleration
        if absolute_pulses < 0: abs_pulse_val_for_pack = (1 << 24) + absolute_pulses
        else: abs_pulse_val_for_pack = absolute_pulses

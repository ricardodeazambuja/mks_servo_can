...
class MultiAxisController:
    ...
    def add_axis(self, axis: Axis) -> None:
        ...
        for existing_axis in self.axes.values():
            if existing_axis.can_id == axis.can_id:
                raise ConfigurationError(
                    f"CAN ID {axis.can_id} for axis '{axis.name}' conflicts with existing axis '{existing_axis.name}'."
                )
    ...
    async def _execute_on_axes(
        ...
        if concurrent and tasks:
            ...
            for task in done:
                axis_name_from_task = task.get_name().split("_")[
                    0
                ]  # Extract axis name
                try:
    ...
    async def move_all_to_positions_abs_user(
        ...
        for axis_name, target_pos in positions_user.items():
            ...
            if not axis:
                logger.warning(
                    f"Axis '{axis_name}' not found in controller for multi-move."
                )
                continue
            ...
            tasks.append(
                self._loop.create_task(
                    axis.move_to_position_abs_user(
                        target_pos, speed_user=axis_speed_user, wait=False
                    ),  # wait=False to not block here
                    name=f"{axis_name}_move_abs",
                )
            )
            active_futures[axis_name] = (
                axis._active_move_future
            )  # Get the future set by the move command
        ...
        if wait_for_all and active_futures:
            ...
            completion_tasks = [
                fut for fut in active_futures.values() if fut
            ]  # Filter out None futures
            if completion_tasks:
                await asyncio.gather(
    ...
    async def move_all_relative_user(
        ...
        for axis_name, rel_dist in distances_user.items():
            ...
            if not axis:
                logger.warning(
                    f"Axis '{axis_name}' not found for multi-relative move."
                )
                continue
            ...
            tasks.append(
                self._loop.create_task(
                    axis.move_relative_user(
                        rel_dist, speed_user=axis_speed_user, wait=False
                    ),

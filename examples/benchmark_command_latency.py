...
DEFAULT_CAN_INTERFACE_TYPE = 'canable'  # e.g., 'socketcan', 'kvaser', 'pcan', 'usb2can'
...
async def benchmark_read_position_low_level(low_level_api: "LowLevelAPI", can_id: int, iterations: int) -> list[float]:
    """Benchmarks reading position using the low-level API directly."""
    latencies = []
    logger.info(f"Benchmarking Low-Level read_encoder_value_addition ({iterations} iterations)...")
    ...
    for i in range(iterations):
        ...
        if (i + 1) % (iterations // 10 or 1) == 0:
            logger.debug(f"Low-level read iteration {i+1}/{iterations} completed.")
        await asyncio.sleep(0.001) # Small delay between commands
    return latencies

async def benchmark_axis_get_position(axis: "Axis", iterations: int) -> list[float]:
    """Benchmarks reading position using the high-level Axis.get_current_position_steps()."""
    latencies = []
    logger.info(f"Benchmarking Axis.get_current_position_steps ({iterations} iterations)...")
    ...
    for i in range(iterations):
        ...
        latencies.append((end_time - start_time) * 1000)  # ms
        if (i + 1) % (iterations // 10 or 1) == 0:
            logger.debug(f"Axis get_position iteration {i+1}/{iterations} completed.")
        await asyncio.sleep(0.001)
    return latencies

async def benchmark_axis_enable_disable(axis: "Axis", iterations: int) -> list[float]:
    """Benchmarks enabling and then disabling the motor using Axis methods."""
    latencies = []
    logger.info(f"Benchmarking Axis enable/disable cycle ({iterations} iterations)...")
    for i in range(iterations):
        start_time = time.perf_counter()
        try:
        ...
        latencies.append((end_time - start_time) * 1000)  # ms
        if (i + 1) % (iterations // 10 or 1) == 0:
            logger.debug(f"Axis enable/disable iteration {i+1}/{iterations} completed.")
        await asyncio.sleep(0.005) # Slightly longer pause after a cycle
    return latencies

async def benchmark_short_relative_move(axis: "Axis", iterations: int, move_pulses: int = 100) -> list[float]:
    """Benchmarks a short relative move and waits for completion."""
    latencies = []
    logger.info(f"Benchmarking Axis short relative move ({move_pulses} pulses, {iterations} iterations)...")
    await axis.enable_motor()
    ...
    for i in range(iterations):
        ...
        try:
            start_time = time.perf_counter()
            await axis.move_relative_pulses(move_pulses, speed_param=1000, accel_param=150, wait=True)
            # await axis.move_relative_pulses(-move_pulses, speed_param=1000, accel_param=150, wait=True) # Move back
            end_time = time.perf_counter()
            latencies.append((end_time - start_time) * 1000)  # ms
            if (i + 1) % (iterations // 10 or 1) == 0:
                logger.debug(f"Axis short move iteration {i+1}/{iterations} completed.")
...
async def run_benchmarks(args: argparse.Namespace):
    ...
    if args.use_simulator:
        ...
        logger.info(f"Using Simulator. Ensure it's running (e.g., with latency ~{args.simulator_latency_ms}ms).")
        ...
        can_if = CANInterface(interface_type=args.can_interface_type,
                              channel=args.can_channel,
                              bitrate=args.can_bitrate,
                              use_simulator=False)
        logger.info(f"Using Real Hardware: {args.can_interface_type} on {args.can_channel} @ {args.can_bitrate}bps.")
    ...
    axis = Axis(can_interface_manager=can_if,
    ...
    try:
        ...
        await axis.initialize(calibrate=False, home=False) # Basic communication check
    ...
    try:
        ...
        if "read_low_level" in args.benchmarks:
            all_results["Low-Level Read Position"] = await benchmark_read_position_low_level(
                low_level_api, args.motor_can_id, args.iterations
            )
        if "read_axis" in args.benchmarks:
            all_results["Axis Read Position (get_current_position_steps)"] = await benchmark_axis_get_position(
                axis, args.iterations
            )
        if "enable_disable" in args.benchmarks:
            all_results["Axis Enable-Disable Cycle"] = await benchmark_axis_enable_disable(
                axis, args.iterations
            )
        if "short_move" in args.benchmarks:
            all_results["Axis Short Relative Move (100 pulses)"] = await benchmark_short_relative_move(
        ...
        for test_name, latencies in all_results.items():
            if not latencies:
                logger.info(f"{test_name}: No data (all iterations failed or benchmark not run).")
                continue
            ...
            if args.use_simulator:
                logger.info(f"  (Simulator Note: Bus latency is configured within the simulator instance, e.g., --latency-ms {args.simulator_latency_ms} for round trip)")

    except KeyboardInterrupt:
        logger.info("Benchmark run interrupted by user.")
    except Exception as e:
        logger.error(f"An unexpected error occurred during benchmarking: {e}", exc_info=True)
    finally:
        logger.info("Cleaning up and disconnecting CAN interface...")
        if axis.is_enabled(): # Attempt to disable the motor if it was left enabled
            try:
                await axis.disable_motor()
            except Exception as e:
...
def main_cli():
    parser = argparse.ArgumentParser(description="MKS Servo CAN Command Latency Benchmarker.")
    parser.add_argument('--use-simulator', action='store_true', default=DEFAULT_USE_SIMULATOR,
                        help="Use the simulator instead of real hardware.")
    parser.add_argument('--no-simulator', action='store_false', dest='use_simulator',
                        help="Use real hardware.")
    parser.add_argument('--simulator-host', default=DEFAULT_SIMULATOR_HOST, help="Simulator host.")
    parser.add_argument('--simulator-port', type=int, default=DEFAULT_SIMULATOR_PORT, help="Simulator port.")
    parser.add_argument('--simulator-latency-ms', type=float, default=DEFAULT_SIMULATOR_LATENCY_MS,
                        help="Expected simulator round-trip latency (for informational display).")

    parser.add_argument('--can-interface-type', default=DEFAULT_CAN_INTERFACE_TYPE, help="CAN interface type for python-can.")
    parser.add_argument('--can-channel', default=DEFAULT_CAN_CHANNEL, help="CAN interface channel.")
    parser.add_argument('--can-bitrate', type=int, default=DEFAULT_CAN_BITRATE, help="CAN bus bitrate.")

    parser.add_argument('--motor-can-id', type=int, default=DEFAULT_MOTOR_CAN_ID, help="CAN ID of the target motor.")
    parser.add_argument('--iterations', type=int, default=DEFAULT_NUM_ITERATIONS, help="Number of iterations for each benchmark.")
    parser.add_argument('--log-level', default=DEFAULT_LOG_LEVEL, choices=['DEBUG', 'INFO', 'WARNING', 'ERROR'],
                        help="Logging level.")
    parser.add_argument('--benchmarks', nargs='+', default=['read_low_level', 'read_axis', 'enable_disable', 'short_move'],
    ...
    if args.can_interface_type == DEFAULT_CAN_INTERFACE_TYPE and \
       args.can_channel == DEFAULT_CAN_CHANNEL and \
       not hasattr(args, 'no_simulator_explicitly_set'): # A way to check if --no-simulator was used
        if not args.use_simulator and not (args.can_interface_type != DEFAULT_CAN_INTERFACE_TYPE or args.can_channel != DEFAULT_CAN_CHANNEL):

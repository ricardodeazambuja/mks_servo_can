...
class EnhancedHeightMapGenerator(MotorDigitizer):
    ...
    def __init__(self, can_interface: CANInterface, config: Optional[Dict[str, Any]] = None):
        """
    ...
    async def setup_standard_plotter(self, motor_ids: Optional[List[int]] = None) -> None:
        """
    ...
    async def interactive_height_mapping(self, save_recording: Optional[str] = None) -> SurfaceMap:
        ...
        print("="*70)
        print("Instructions:")
        print("  1. Motors will be disabled for manual control")
        print("  2. Move the pen to desired locations manually")
        print("  3. At each location, press commands to record height")
        print("  4. Recording will capture the complete session")
        print("-"*70)
        ...
        try:
            ...
            while True:
                ...
                print("Position the pen tip at desired measurement location")
                print("Commands: (m)easure point, (f)inish mapping, (q)uit")
    ...
    async def grid_height_mapping(self, grid_spacing: float = 10.0,
                                bounds: Optional[Tuple[float, float, float, float]] = None) -> SurfaceMap:
        """
        ...
        try:
            ...
            for i, x in enumerate(x_points):
                for _j, y in enumerate(y_points):
                    point_num += 1
                    ...
                    try:
                        ...
                        if not (math.isnan(z_height) or math.isnan(pen_angle)):
                            ...
                            print("   ⚠️  Failed to measure point")

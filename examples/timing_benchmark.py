...
async def benchmark_read_position(axis: Axis, iterations: int) -> list[float]:
    ...
    latencies = []
    logging.info(
        f"Benchmarking read_encoder_value_addition ({iterations} iterations)..."
    )
    ...
    for i in range(iterations):
        ...
        await asyncio.sleep(
            0.001
        )  # Small delay to prevent overwhelming bus/sim
    return latencies
...
async def benchmark_enable_disable(axis: Axis, iterations: int) -> list[float]:
    ...
    for i in range(iterations):
        ...
        try:
            ...
            logging.warning(
                f"Iteration {i+1}: Error during enable/disable: {e}"
            )
            continue
        end_time = time.perf_counter()
...
async def main():
    if USE_SIMULATOR:
        ...
        logging.info(
            f"Using Simulator. Ensure it's running and configured (e.g., with latency {sim_latency_ms}ms)."
        )
    else:
        can_if = CANInterface(
        ...
        logging.info(
            f"Using Real Hardware: {CAN_INTERFACE_TYPE} on {CAN_CHANNEL} @ {CAN_BITRATE}bps."
        )
    ...
    try:
        ...
        for test_name, latencies in all_results.items():
            if not latencies:
                logging.info(
                    f"{test_name}: No data collected (all iterations failed or skipped)."
                )
                continue
            count = len(latencies)
            ...
            if USE_SIMULATOR:
                logging.info(
                    f"  (Note: Simulator introduces its own configured latency of approx. {can_if.simulator_host if hasattr(can_if, 'simulator_host') else 'N/A'} bus latency.)"
                )  # TODO: Get actual sim latency if possible

    except KeyboardInterrupt:
        logging.info("Benchmark interrupted by user.")
    except Exception as e:
        logging.error(
            f"An error occurred during benchmarking: {e}", exc_info=True
        )
    finally:

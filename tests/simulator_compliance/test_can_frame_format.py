...
class TestCANFrameFormat:
    ...
    def test_arbitration_id_format(self):
        ...
        for motor_id in motor_ids:
            ...
            assert downlink_id == 0x140 + motor_id, "Incorrect downlink ID calculation"
            assert uplink_id == 0x240 + motor_id, "Incorrect uplink ID calculation"
    ...
    def test_data_field_formats(self):
        ...
        for cmd_code, cmd_spec in commands.items():
            ...
            if cmd_code == "0x30":  # Encoder carry format
                assert "bytes1-4" in response_fields, "0x30 missing carry field"
                assert "bytes5-6" in response_fields, "0x30 missing value field"
                assert response_fields["bytes1-4"] == "carry_int32", "0x30 carry should be int32"
                assert response_fields["bytes5-6"] == "value_uint16", "0x30 value should be uint16"

            elif cmd_code == "0x31":  # Encoder addition format
                assert "bytes1-6" in response_fields, "0x31 missing value field"
                assert response_fields["bytes1-6"] == "value_int48", "0x31 value should be int48"

            elif cmd_code == "0x32":  # Speed format
                assert "bytes1-2" in response_fields, "0x32 missing speed field"
                assert response_fields["bytes1-2"] == "speed_int16_rpm", "0x32 speed should be int16"
    ...
    def test_integer_types_specification(self):
        ...
        for cmd_code, _field_name, expected_type in test_cases:
            if cmd_code in commands:
                ...
                found_type = False
                for _field_key, field_desc in response_fields.items():

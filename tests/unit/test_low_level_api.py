...
class TestLowLevelAPIReads:
    async def test_read_encoder_value_addition_success(
        ...
        expected_sent_data_bytes = bytes(
            expected_sent_payload_data + [expected_sent_crc]
        )
    ...
    async def test_read_command_response_crc_error(
        ...
        with pytest.raises(
            CRCError,
            match=f"Invalid CRC in response to command {command_code:02X}",
        ):
            await low_level_api.read_encoder_value_addition(can_id)
    ...
    async def test_read_command_response_dlc_mismatch(
        ...
        with pytest.raises(
            CommandError,
            match=f"Response DLC mismatch for command {command_code:02X}",
        ):
            await low_level_api.read_encoder_value_addition(can_id)
...
class TestLowLevelAPISets:
    ...
    async def test_calibrate_encoder_failure(
        ...
        with pytest.raises(
            CalibrationError, match="Encoder calibration failed."
        ):
...
class TestLowLevelAPIReadSystemParameter:
    async def test_read_system_parameter_success(
        ...
        expected_sent_payload_data = [
            const.CMD_READ_SYSTEM_PARAMETER_PREFIX,
            param_to_read_cmd_code,
        ]
        expected_sent_crc = (can_id + sum(expected_sent_payload_data)) & 0xFF
        expected_sent_data_bytes = bytes(
            expected_sent_payload_data + [expected_sent_crc]
    ...
    async def test_read_system_parameter_not_readable(
        ...
        with pytest.raises(
            MotorError,
            match=f"Parameter with code {param_to_read_cmd_code:02X} cannot be read",
        ):
